  ...
  checkStatusHandler() {
    ...
    fastFactory
      ...
      .then((fasts) => {
        fasts.forEach((fast) => {
          if (fast.active) {
            // checking for active fast
            let nowDateUTC = new Date().toISOString();

            if (fast.end <= nowDateUTC) {
              // the fasting has already ended
              fast.active = false;

              console.log(
                "Your last fast has already ended. Here are the details:"
              );
              ...
              console.log(
                `Hours spent in fasting: ${
                  new Date(fast.end).getHours() -
                  new Date(fast.start).getHours()
                }`
              );
              ...
              // the fasting is still active
              console.log("You have an ongoing fast:");
              console.log("Start time: ", new Date(fast.start).toString());
              console.log("End time: ", new Date(fast.end).toString());
              console.log(
                `Hours elapsed: ${
                  new Date(nowDateUTC).getHours() -
                  new Date(fast.start).getHours()
                }`
              );
            }
